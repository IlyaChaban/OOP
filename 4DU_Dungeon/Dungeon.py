...
class Entity:
    ...
    def __init__(self, name: str, coords: Tuple[int, int], hit_points: int) -> None:
        ...
        # coords is tuple to represent x and y coordinates in the field: (1, 2) menas x = 1 and y = 2; expect also
        # negative values
        #
    ...
    def get_distance(self, other_coords: Tuple[int, int]) -> int:
        ...
        distance = (self.coords[0] - other_coords[0]) + (self.coords[1] - other_coords[1]) if (self.coords[0] - other_coords[0]) + (self.coords[1] - other_coords[1]) >= 0 else ((self.coords[0] - other_coords[0]) + (self.coords[1] - other_coords[1])) * -1
        return distance
...
class Rock(Entity):
    ...
    def take_damage(self, damage_amount: int) -> None:
        ...
        # each such hit takes him 1 hit point, smaller hits don't cause any damage to him
        self.hit_points -= 1 if damage_amount >= 10 else 0
...
class LivingEntity(Entity):
    ...
    def __init__(self, name: str, coords: Tuple[int, int], hit_points: int, level: int, damage: int,
    ...
    def in_range(self, other_entity: Entity) -> bool:
        ...
        if other_entity.get_distance(self.coords) <= self.attack_range:
            return True
        else:
            return False
...
class Warrior(LivingEntity):
    ...
    def hit(self, other_entity: Entity) -> None:
        ...
        #   if other entity is in range make it to take damage with its own method take_damage
        #       damage_amount applied to other entity is equal to damage to attacking entity
        if other_entity.get_distance(self.coords) < self.attack_range:
    ...
    def take_damage(self, damage_amount: int) -> None:
        ...
        self.hit_points -= (damage_amount-self.level if self.level < damage_amount else 0)
